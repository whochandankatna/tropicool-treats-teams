...
export async function squareFetch(token: string, path: string, init: RequestInit = {}): Promise<any> {
  ...
  if (!res.ok) { const msg = json?.errors?.map((e: any) => [e.category, e.code, e.field, e.detail].filter(Boolean).join(": ")).join("; ") || res.statusText; throw new Error(`Square API error (${res.status}): ${msg}`); }
  return json;
}
...
export async function requireManagerPin(db: any, pin: string | undefined | null) {
  ...
  const match = (data ?? []).find((r: any) => r.pin != null && r.pin === String(pin));
...
export async function sendPushToTeamIds(db: any, teamIds: number[], payload: { title: string; body: string; tag?: string }, managerVerifiedOnly = false, delivery: { TTL?: number; urgency?: "normal" | "high" } = {}): Promise<number> {
  ...
  let query = db.from("tt_push_subscriptions").select("id, team_id, endpoint, p256dh, auth").in("team_id", teamIds);
  if (managerVerifiedOnly) query = query.eq("verified_manager", true);
  const { data: subs } = await query;
  let sent = 0;
  for (const s of subs ?? []) {
    try { await webpush.sendNotification({ endpoint: s.endpoint, keys: { p256dh: s.p256dh, auth: s.auth } }, JSON.stringify(payload), delivery); sent++; }
